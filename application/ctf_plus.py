...
def _build_oauth_otp_callback(*, email: str, proxy: str = "", log_fn=None):
    from core.base_mailbox import MailboxAccount, create_mailbox
    from infrastructure.provider_settings_repository import ProviderSettingsRepository

    log = log_fn or (lambda _message: None)
    provider_key = ProviderSettingsRepository().get_default_provider_key("mailbox")
    provider_key = str(provider_key or "").strip()
    if not provider_key:
        log("  OAuth email OTP disabled: no default mailbox provider")
        return None
    mailbox = create_mailbox(provider=provider_key, extra={}, proxy=proxy or None)
    mail_account = MailboxAccount(
        email=str(email or "").strip(),
        account_id=str(email or "").strip(),
        extra={"mailbox_provider_key": provider_key},
    )

    def otp_callback():
        log(f"  OAuth waiting email OTP: {mail_account.email}")
...
class CtfPlusAccountsService:
    ...
    def start_codex_oauth(self, *, account_id: int) -> dict[str, Any]:
        ...
        self._oauth_sessions[int(account_id)] = session
        overview = dict(account.overview or {})
        overview["codex_oauth_pending"] = {
            "state": session.state,
        ...
        self.repository.update(int(account_id), AccountUpdateCommand(overview=overview))
        return {
            "account_id": int(account_id),
            "email": account.email,
    ...
    def complete_codex_oauth(self, *, account_id: int, callback_url: str) -> dict[str, Any]:
        ...
        if not payload.get("access_token"):
            ...
            if not session:
                pending = dict((account.overview or {}).get("codex_oauth_pending") or {})
        ...
        account_after = self.repository.get(int(account_id))
        cpa_result = self._upload_cpa_after_oauth(account_after) if account_after else {"ok": False, "message": "account reload failed"}
        return {
            "ok": True,
            "account_id": int(account_id),
            "email": account.email,
            "refresh_token_present": bool(payload.get("refresh_token") or payload.get("refreshToken")),
            "id_token_present": bool(payload.get("id_token") or payload.get("idToken")),
            "cpa_upload": cpa_result,
        }
    ...
    def run_codex_oauth_browser(
        ...
        *,
        ...
        log = log_fn or (lambda _message: None)
        otp_callback = _build_oauth_otp_callback(email=account.email, proxy=proxy, log_fn=log)
        acquired_profile_id = ""
    ...
    def _upload_cpa_after_oauth(self, account) -> dict[str, Any]:
        if not account:
            return {"ok": False, "message": "account reload failed"}
        try:
            from platforms.chatgpt.cpa_upload import generate_token_json, upload_to_cpa

            token_data = generate_token_json(account)
            ok, message = upload_to_cpa(token_data)
            return {
                "ok": bool(ok),
                "message": str(message),
