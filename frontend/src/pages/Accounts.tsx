...
function hasRefreshToken(acc: any) {
  if (typeof acc?.has_refresh_token === 'boolean') return acc.has_refresh_token
  return getCredentials(acc).some((item: any) => {
    const key = String(item?.key || '')
    const value = String(item?.value || '').trim()
    return item?.scope === 'platform' && (key === 'refresh_token' || key === 'refreshToken') && value.length > 0
  })
}
...
function RegisterModal({
  ...
  const [gopayPin, setGopayPin] = useState('000000')
...
export default function Accounts() {
  ...
  const visibleWithRefresh = accounts.filter(hasRefreshToken).length
  const visibleMissingRefresh = accounts.length - visibleWithRefresh
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-5 py-4 border-b border-[var(--border)]/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-xs">
              ...
              {tab === 'chatgpt' && visibleWithRefresh > 0 && <span className="flex items-center rounded-full bg-cyan-500/10 px-2 py-0.5 font-medium text-cyan-400 ring-1 ring-inset ring-cyan-500/20">Refresh Token: {visibleWithRefresh}</span>}
              {tab === 'chatgpt' && visibleMissingRefresh > 0 && <span className="flex items-center rounded-full bg-zinc-500/10 px-2 py-0.5 font-medium text-zinc-400 ring-1 ring-inset ring-zinc-500/20">No Refresh: {visibleMissingRefresh}</span>}
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-2.5 bg-[var(--bg-pane)]/20">
          <div className="flex flex-1 items-center gap-3">
            ...
            >
              ...
              {tab === 'chatgpt' && <option value="has_refresh_token">Has refresh_token</option>}
              {tab === 'chatgpt' && <option value="missing_refresh_token">Missing refresh_token</option>}
            </select>
          </div>
              ...
              <tr key={acc.id} className="group border-b border-[var(--border)]/30 hover:bg-[var(--text-primary)]/[0.02] transition-colors cursor-pointer"
                ...
                <td className="px-3 py-2.5 align-top">
                  <div className="min-w-0 flex flex-col items-start gap-1.5">
                    ...
                    {tab === 'chatgpt' && (
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${hasRefreshToken(acc) ? 'bg-cyan-500/10 text-cyan-400 ring-cyan-500/20' : 'bg-zinc-500/10 text-zinc-400 ring-zinc-500/20'}`}
                        title={hasRefreshToken(acc) ? 'Refresh token present' : 'No refresh token saved'}
                      >
                        {hasRefreshToken(acc) ? 'Refresh OK' : 'No Refresh'}
                      </span>
                    )}
                    {primaryMetrics.length > 0 ? (
