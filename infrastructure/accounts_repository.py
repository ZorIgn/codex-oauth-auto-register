...
def _has_refresh_token(credentials: list[dict]) -> bool:
    for credential in credentials or []:
        key = str(credential.get("key") or "")
        if key not in {"refresh_token", "refreshToken"}:
            continue
        if str(credential.get("value") or "").strip():
            return True
    return False


def _matches_account_filter(item: AccountRecord, status: str) -> bool:
    expected = str(status or "").strip()
    if expected == "has_refresh_token":
        return item.has_refresh_token
    if expected == "missing_refresh_token":
        return not item.has_refresh_token
    return matches_status_filter(
        {
...
def _to_record(model: AccountModel, graph: dict | None = None) -> AccountRecord:
    ...
    return AccountRecord(
        ...
        has_refresh_token=_has_refresh_token(credentials),
...
class AccountsRepository:
    ...
    def list(self, query: AccountQuery) -> tuple[int, list[AccountRecord]]:
        ...
        with Session(engine) as session:
            ...
            if query.status:
                records = [item for item in records if _matches_account_filter(item, query.status)]
        total = len(records)
    ...
    def select_for_export(self, selection: AccountExportSelection) -> list[AccountRecord]:
        ...
        if selection.status_filter:
            records = [item for item in records if _matches_account_filter(item, selection.status_filter)]
        return records
