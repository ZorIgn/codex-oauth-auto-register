...
def test_build_otp_initiate_request_matches_har_layout():
    ...
    assert vars_["phoneNumber"] == "2025550100"
...
def test_build_otp_initiate_request_validates_inputs():
    ...
    with pytest.raises(ValueError):
        paypal_http.build_otp_initiate_request(ec_token="", phone_number_local="2025550100")
    with pytest.raises(ValueError):
        paypal_http.build_otp_initiate_request(ec_token="bogus", phone_number_local="2025550100")
    with pytest.raises(ValueError):
...
def test_paypal_post_otp_initiate_uses_correct_endpoint_and_headers():
    ...
    session = _StubJsonSession(payload)
    body = paypal_http.build_otp_initiate_request(
        ec_token="EC-A", phone_number_local="2025550100",
    )
...
def test_paypal_post_otp_initiate_and_confirm_share_explicit_cmid():
    ...
    paypal_http.paypal_post_otp_initiate(
        init_session,
        body=paypal_http.build_otp_initiate_request(ec_token="EC-A", phone_number_local="2025550100"),
        ec_token="EC-A",
        ba_token="BA-X",
