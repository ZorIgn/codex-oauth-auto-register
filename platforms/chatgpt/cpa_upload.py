...
def _safe_cpa_filename(email: str) -> str:
    base = re.sub(r"[^A-Za-z0-9_.-]+", "_", str(email or "").strip())
    base = base.strip("._") or "chatgpt-account"
    if not base.lower().startswith("codex-"):
        base = f"codex-{base}"
...
def _load_cpa_auth_dir() -> Path:
    candidates = [
        Path(r"D:\downloads\CLIProxyAPI_7.2.7_windows_amd64\config.yaml"),
        ...
        try:
            ...
            for line in config_path.read_text(encoding="utf-8", errors="ignore").splitlines():
                if line.strip().startswith("auth-dir:"):
                    ...
                    if value:
                        value = value.replace("~", str(Path.home()), 1)
...
def write_cpa_auth_file(token_data: dict, auth_dir: str = "") -> Tuple[bool, str]:
    try:
        target_dir = Path(auth_dir).expanduser() if auth_dir else _load_cpa_auth_dir()
        target_dir.mkdir(parents=True, exist_ok=True)
        target_path = target_dir / _safe_cpa_filename(token_data.get("email", ""))
        target_path.write_text(
            json.dumps(token_data, ensure_ascii=False, separators=(",", ":")),
            encoding="utf-8",
        )
        return True, f"Wrote CPA auth file: {target_path}"
    except Exception as exc:
        return False, f"CPA auth-dir write failed: {exc}"
...
def upload_to_cpa(
    ...
    try:
        ...
        if response.status_code == 500 or "failed to write file" in error_msg.lower():
            fallback_ok, fallback_msg = write_cpa_auth_file(token_data)
            if fallback_ok:
                return True, fallback_msg
            return False, f"{error_msg}; fallback failed: {fallback_msg}"
